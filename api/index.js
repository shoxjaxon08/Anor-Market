  ...
  const sendVerificationTokenEmail = async (email, verificationToken) => {
    ...
    const verificationUrl = `http://192.168.70.10:8000/verify/${verificationToken}`;
    const mailOptions = {
  ...
  app.put("/addresses/:userId/:id", async (req, res) => {
    try {
      ...
      const token = authHeader.split(" ")[1];
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      console.log("Serverga kelgan headers:", req.headers.authorization);
      console.log("Serverga kelgan body:", req.body);
  ...
  app.post("/orders", async (req, res) => {
    try {
      ...
      if (cartItems.length === 0) {
        return res.status(400).json({ message: "Savat bo‘sh, buyurtma berish mumkin emas." });
      }
      ...
      res.status(500).json({ message: "Buyurtma yaratishda server xatoligi yuz berdi" });
    }
  });
...
//profile get api
  app.get("/profile/:userId",async (req,res) => {
     try {
      ...
      res.status(500).json({message:"Profile olishda xatolik",error})
     }
  } )
  ...
  app.get("/orders/:userId",async (req,res) => {
    try {
     ...
     if(!orders || orders.length === 0){
       return res.status(404).json({message:"Foydalanuvchining buyurtmasi yoq"})
     }
     res.status(200).json({orders})
    } catch (error) {
      res.status(500).json({message:"Buyurtmalarni olishda xatolik",error})
    }
 } )
